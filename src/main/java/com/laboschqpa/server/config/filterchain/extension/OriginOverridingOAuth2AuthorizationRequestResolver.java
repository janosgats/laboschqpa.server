...
public final class OriginOverridingOAuth2AuthorizationRequestResolver implements OAuth2AuthorizationRequestResolver {
    private static final String REGISTRATION_ID_URI_VARIABLE_NAME = "registrationId";
    private static final char PATH_DELIMITER = '/';

    private static final String OAUTH2_OVERRIDE_REDIRECTION_ORIGIN_HEADER_NAME = "X-Oauth2-Override-Redirection-Origin";
    ...
    private List<String> allowedOverriddenRedirectionOrigins;

    public OriginOverridingOAuth2AuthorizationRequestResolver(ClientRegistrationRepository clientRegistrationRepository,
                                                              String authorizationRequestBaseUri,
                                                              List<String> allowedOverriddenRedirectionOrigins) {
        Assert.hasText(authorizationRequestBaseUri, "authorizationRequestBaseUri cannot be empty");
        Assert.notNull(clientRegistrationRepository, "clientRegistrationRepository cannot be null");
        Assert.notNull(allowedOverriddenRedirectionOrigins, "allowedOverriddenRedirectionBaseUrls cannot be null");

        this.allowedOverriddenRedirectionOrigins = allowedOverriddenRedirectionOrigins;
    ...
    private String expandRedirectUri(HttpServletRequest request, ClientRegistration clientRegistration,
        ...
        UriComponents uriComponents = UriComponentsBuilder.fromHttpUrl(getOauth2RedirectionUrl(request))
    ...
    private String getOauth2RedirectionUrl(HttpServletRequest request) {
        final String overriddenOriginFromHeader = request.getHeader(OAUTH2_OVERRIDE_REDIRECTION_ORIGIN_HEADER_NAME);

        if (StringUtils.isNotBlank(overriddenOriginFromHeader)
                && allowedOverriddenRedirectionOrigins.contains(overriddenOriginFromHeader)) {
            return overriddenOriginFromHeader + request.getContextPath();
        }
